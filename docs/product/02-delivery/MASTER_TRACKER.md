...
| Steps `VERIFIED` | **6** (STEP-001…006) — corrected at the STEP-007 close. This line read **4** while §7 already showed `STEP-005` at 10/10 and `STEP-006` at 9/9: the summary had drifted from the table beneath it, in the one document rule 10 makes authoritative |
...
> **The rest of §1 is stale and is not silently rewritten here.** "Application code
> files: **0** product code" predates every handler in `apps/api`; "Open blockers:
> **1** (`BLK-002`)" predates that blocker's closure; "Requirements verified against
> implementation: **0**" has never been recomputed. Each needs a count somebody
> derives and owns, not a number invented while closing an unrelated step — so they are
> **flagged as owed** rather than adjusted. §7, the step tracker, is accurate.
...
| ~~BLK-002~~ | ~~No application code exists~~ — **PARTIALLY CLOSED 2026-08-12** (STEP-002.02 onward): 985 Python tests across `apps/api`, four services and `tools/` | Contracts remain `PROPOSED` — no handler implements one. Graph coverage gates are now evaluable and **`RISK-016` says one of them under-reports** | Engineering | Contracts: before `STEP-006` |
| ~~DEC-002~~ | ~~Phase 1 destination region~~ — **CLOSED 2026-08-13: Switzerland** (`ADR-016`) | **`RISK-001` is not closed by it.** OSM is ODbL and the evidence pack is a derivative database, so a posture is owed **before STEP-010** | Product Lead | Resolved |
| ~~DEC-004~~ | ~~Identity provider~~ — **CLOSED 2026-08-06: Auth0** (`ADR-013`) | **Unverified against a live tenant** — no Auth0 account exists, so passkey enrolment and rotation-under-concurrency are unproven (`BR-014` §9) | Security Architect | Resolved |
...
| ~~DEC-008~~ | ~~Routing provider~~ — **CLOSED 2026-08-17: OpenTripPlanner 2, self-hosted** (`ADR-018`) | Self-hosting arrives before Phase 1 and couples to `DEC-007`. Accepted knowingly | Product Architect | Resolved |
| ~~DEC-009~~ | ~~Event backbone~~ — **CLOSED 2026-08-13: Kafka** (`ADR-015`), owner directive | Unblocks AsyncAPI client generation and compatibility diffing, both of which named this decision as their trigger | Product Architect | Resolved |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [STEP-001](../08-steps/STEP-001-foundation-and-repository-governance.md) | Foundation & repo governance | 1 | Reproducible monorepo with ownership and contract boundaries | — | **`VERIFIED`** | Deepesh Kumar Gupta | ✅ | ✅ | — | — | — | ✅ | partial | ✅ BR-001…007 | ✅ 8/8 | ✅ 19 guards + 68 meta | ✅ | `8dd20f4` | New engineer runs lint/tests/app from docs; CI rejects unowned changes | BLK-001, BLK-002. **.07 added 2026-08-13** — CI had never run R7 (`BUG-023`) | 2026-08-13 |
| [STEP-002](../08-steps/STEP-002-identity-tenancy-and-authorization.md) | Identity, tenancy & authorization | 1 | Isolation and permission primitives | 001 | **`VERIFIED`** 8/8 | Deepesh Kumar Gupta | NS | ✅ .01–.08 | **✅ .05 (auth only)** | ✅ .01, .04 | — | ✅ R7 18/18 | ✅ **RUNNABLE** (BR-012, BR-013) | ✅ BR-008, BR-010…017, BR-036 | ✅ 8/8 | ✅ 665 py + 63 ts + 18 R7 | ✅ | — | Cross-tenant ops fail deterministically and are audited | ~~`DEC-004`~~ **CLOSED — Auth0**; `DEC-010` (fails closed); **REQ-TRIP-005 open** (STEP-007); **REQ-SEC-003 partial** (no live Auth0 tenant; a11y needs STEP-003) | 2026-08-06 |
| [STEP-003](../08-steps/STEP-003-design-system-and-application-shell.md) | Design system & app shell | 1 | Accessible responsive shell and primitives | 002 | **`VERIFIED`** 9/9 | Deepesh Kumar Gupta | ✅ .01–.09 | — | — | — | — | ✅ | ✅ **RUNNABLE** (BR-018…026) — but component impact is **unreliable**: JSX usage is not traced, and CSS is not in the graph at all (BR-025 §3, BR-026 §3) | ✅ BR-018…026 | ✅ 9/9 | ✅ **311 UI** + 61 web + **40 browser** | ✅ | — | WCAG 2.2 AA on core components; all states implemented; **axe, keyboard, touch targets, forced-colors, RTL and CWV gate the build** | **BUG-018 closed. BUG-033 closed 2026-09-09** (`BR-062`) — `DataTable`'s scroll region is focusable and the CSV control sits outside it; **axe passed throughout the defect**, so the gate above is necessary and not sufficient. CWV are lab numbers — field measurement is STEP-024. Manual screen-reader journeys **required every release**: see [ACCESSIBILITY_AUTOMATION_LIMITS](../06-quality/ACCESSIBILITY_AUTOMATION_LIMITS.md). **.09 added post-review** — the plan had no design sub-step; icons, a logo and a non-implementer design review remain open | 2026-09-09 |
| [STEP-004](../08-steps/STEP-004-contract-first-platform-apis.md) | Contract-first platform APIs | 1 | Stable resource/command/event contracts | 002 | **`VERIFIED`** 9/9 | Deepesh Kumar Gupta | ✅ .01–.07 | — | ✅ **`contracts/openapi.yaml` exists and is authoritative** (ADR-001) | — | — | ✅ .01 | ✅ **RUNNABLE** — `impact(opaque_denial)` traced 2 callers and 1 flow, the first useful graph answer here | ✅ BR-028…035, BR-040 | ✅ 9/9 | ✅ 736 py + 61 web + 307 UI + 40 browser | ✅ | — | ✅ **CI generates clients, blocks drift and blocks a breaking change without a major version bump** (.07, .08); validates examples (.02) | **Two error shapes coexist** — `auth/errors.py` is not yet RFC 9457, migrates at .04. **BUG-020** (.07): a retained evidence conflict could not name its source — found by generating the client, not by 592 tests reading the same YAML. **BUG-021** (.08): two guarantees depended on optional fields — found by the promised audit for existence-only assertions. **AsyncAPI is not compatibility-diffed** (`DEC-009` open) and **semantic change is undetectable** (`ENH-001` pending) — both stated, neither hidden. Rate-limit values deferred (`ASM-002`). **`gitnexus_query` is unusable — `--repair-fts` is not a valid flag** (BR-029 §3). `DATA-010/011` referenced but undefined (STEP-006) | 2026-08-11 |
| [STEP-005](../08-steps/STEP-005-source-integrations-and-ingestion.md) | Source integrations & ingestion | 1 | Inputs with consent, provenance, replay | 004 | **`VERIFIED`** 10/10 | Deepesh Kumar Gupta | — | ✅ .01–.10 | ✅ **EVT-008 consumed as declared** (.10) | ✅ .02, .04, .07–.09 | — | ✅ **no provider identity in the public view** (.10); egress allowlist + SSRF (.01) | ✅ **RUNNABLE — and under-reporting, see `RISK-016`** | ✅ BR-038, BR-041…049 | ✅ 10/10 | ✅ 388 Python tests; **59 mutants killed across .06–.10** | ✅ | — | Every source has credentials, limits, checkpoints, reconciliation, deletion | ~~DEC-002~~ **closed 2026-08-13 — Switzerland (`ADR-016`)**. ~~DEC-008~~ **closed 2026-08-17 — OpenTripPlanner 2 self-hosted (`ADR-018`)**, EV-GAP-002 open. **`RISK-001` narrowed, not closed**: OSM is ODbL and the evidence pack is a derivative database — a posture is owed before STEP-010. **`RISK-016` raised at .07**: the code graph reports 0 dependants for symbols with live call sites, so every blast-radius record cross-checks it against a grep | 2026-08-18 |
| [STEP-006](../08-steps/STEP-006-canonical-data-model-and-event-backbone.md) | Canonical model & event backbone | 1 | Versioned entities and replayable events | 005 | **`VERIFIED`** 9/9 | Deepesh Kumar Gupta | — | 🔄 .01 | — | ✅ **15 tables, DATA-003…016** (.01) | — | ✅ **13 RLS policies, 3 immutability triggers, booking segregated by grant** (.01) | **`RISK-017` — the graph cannot see a migration**; blast radius from deployed-schema mutation instead | ✅ BR-050 | ✅ 9/9 | ✅ 219 tests, 117 mutants killed | ✅ | — | Records retain provenance; events rebuild read models | BLK-001, DEC-009 | 2026-08-05 |
| [STEP-007](../08-steps/STEP-007-discovery-landing-and-destination-coverage.md) | Discovery landing & coverage | 1 | Expectations set before signup | 003, 006 | **`IN_REVIEW`** 5/5 — **one manual gate owed** | Deepesh Kumar Gupta | NS | ✅ .01–.05 — **the first product route, the first product page, the first refusal a traveller meets, the first personal data this product has ever held, and the first write from a projection to the table a traveller reads** | ✅ `API-017` as declared, **plus `Coverage.observed_at`**; **`API-019` and `API-020` added** (all additive, gate-classified) | ✅ migrations 016, 017, 018, **019** (`waitlist_entries`, no RLS — exempt by the isolation gate's own derivation, having no `organization_id`) | ✅ .03 announces refusals assertively; .04 consent is never pre-ticked; .05 disclosure is polite and announced once per change, asserted in a real browser; 92 browser | ✅ **NOT BLOCKED** at .05 — `RISK-016` #15: `get_coverage` reported 0 dependants against 8, a cross-file call into `app.py`, the exact shape BR-063 §2 isolated (intra-file edges resolve, cross-file ones do not) | ✅ .03 — 20/20; .04 — 13/13; **.05 — 12/12**; `BUG-036` — 14/14; `BUG-037` — 11/11 — **70 seeded, 70 killed across the step, 0 survivors** | ✅ **BR-059 … BR-066** — sub-steps .01–.05 are BR-059, BR-060, BR-061, BR-063, BR-064; the other three are bugs found and fixed inside the step: BR-062 (`BUG-033`), BR-065 (`BUG-036`), BR-066 (`BUG-037`) | ✅ 5/5 | ✅ 1478 py + 102 web + 311 ui + 92 browser + 18 R7 | ✅ | — | User knows what is supported and what JourneyLab will not do pre-signin, and can ask to be told when that changes | ~~BLK-001~~; **`DEC-011` open** (3-day lower bound shipped as declared, recommended against); **.04 ships a known trade** — a waitlist token can be rotated by anyone who knows the address until email verification exists; **.05 ships a seam, not a pipeline** — nothing runs `apply_coverage_state` in production until an `EVT-008` consumer exists (`ENH-007`); **`BUG-036` fixed at step close** — waitlist addresses had no retention period; now until-sent with a 12-month cap (`DEC-012`), enforceable and **not yet scheduled**; **`BUG-037` fixed at step close** — a region that lost a provider could never recover, live or after a rebuild, and the `EVT-008` dedupe key would have discarded a provider's second outage. **`IN_REVIEW` not `VERIFIED`:** §25's screen-reader criterion needs manual journeys nobody has run, and `VERIFIED` means evidence exists | 2026-09-16 |
