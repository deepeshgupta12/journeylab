...
| [STEP-004](../08-steps/STEP-004-contract-first-platform-apis.md) | Contract-first platform APIs | 1 | Stable resource/command/event contracts | 002 | **`IN_PROGRESS`** 3/8 | Deepesh Kumar Gupta | ✅ .01–.03 | — | ✅ **`contracts/openapi.yaml` exists and is authoritative** (ADR-001) | — | — | ✅ .01 | ✅ **RUNNABLE** — `impact(opaque_denial)` traced 2 callers and 1 flow, the first useful graph answer here | ✅ BR-028…030 | ✅ 3/8 | ✅ 469 py + 61 web + 307 UI + 40 browser | ✅ | — | CI generates clients, validates examples, blocks breaking change | **Two error shapes coexist** — `auth/errors.py` is not yet RFC 9457, migrates at .04. Rate-limit values deferred (`ASM-002`). **`gitnexus_query` is unusable — `--repair-fts` is not a valid flag** (BR-029 §3). `DATA-010/011` referenced but undefined (STEP-006) | 2026-08-11 |
