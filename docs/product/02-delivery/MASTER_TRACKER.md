...
| [STEP-007](../08-steps/STEP-007-discovery-landing-and-destination-coverage.md) | Discovery landing & coverage | 1 | Expectations set before signup | 003, 006 | **`IN_PROGRESS`** 4/5 | Deepesh Kumar Gupta | NS | ✅ .01–.04 — **the first product route, the first product page, the first refusal a traveller meets, and the first personal data this product has ever held** | ✅ `API-017` as declared; **`API-019` and `API-020` added** (additive, gate-classified) | ✅ migrations 016, 017, 018, **019** (`waitlist_entries`, no RLS — exempt by the isolation gate's own derivation, having no `organization_id`) | ✅ .03 announces refusals assertively; .04 consent is never pre-ticked, asserted in a real browser; 76 browser | ✅ **NOT BLOCKED** at .04 — and the graph was *correct* there (BR-063 §2), which corrects BR-061's over-strong claim that `app.py` has no outgoing edges: intra-file edges resolve, cross-file ones do not | ✅ .03 — 20/20; **.04 — 13/13** | ✅ BR-059, BR-060, BR-061, BR-063 | ✅ 4/5 | ✅ 1424 py + 87 web + 311 ui + 76 browser + 18 R7 | ✅ | — | User knows what is supported and what JourneyLab will not do pre-signin, and can ask to be told when that changes | ~~BLK-001~~; **`DEC-011` open** (3-day lower bound shipped as declared, recommended against); **.04 ships a known trade** — a waitlist token can be rotated by anyone who knows the address until email verification exists | 2026-09-11 |
