...
| --- | --- | --- | --- | --- | --- |
| ~~DEC-002~~ | ~~Phase 1 destination region~~ | **CLOSED 2026-08-13 — Switzerland** (`ADR-016`), proposed by the implementer and confirmed by the owner under `ADR-007`. Constraint: open data only, zero licence spend. **`RISK-001` is not closed by this** — it narrows from "unproven licensability" to a specific question: OSM is ODbL and the evidence pack is a derivative database, so a posture (comply / segregate / prefer non-ODbL) is owed **before STEP-010**. Open-Meteo's free tier is non-commercial, so MeteoSwiss is the weather source | — | Product Lead | Resolved at STEP-005 |
...
| ~~DEC-008~~ | ~~Routing provider~~ | **CLOSED 2026-08-17 — OpenTripPlanner 2, self-hosted** (`ADR-018`). Proposed at STEP-005.05 and confirmed by the owner. The only open-source engine routing transit and step-free together, reusing exactly the feeds `ADR-016` chose. **Cost accepted knowingly:** self-hosting arrives before Phase 1 and couples to `DEC-007` | — | Product Architect | Resolved at STEP-005.05 |
...
| DEC-011 | The minimum trip length Phase 1 will plan | `PRODUCT_SCOPE` §81 declares **3–7 days** and `ASM-015` justifies it as "large enough to demonstrate differentiated simulation without unbounded complexity". That reasoning constrains the **upper** bound and says nothing about the lower one — a two-day trip is *less* complex than a three-day one, not more. Options: (a) keep 3, and accept refusing a weekend break; (b) lower to 1, and widen `ASM-015`. **Implemented as declared (3) pending this decision**, because shipping 1–7 on the implementer's preference would be editing product scope through code. One constant, `MIN_TRIP_DAYS`, one line to change | STEP-007.03 refuses a 1–2 day trip today; STEP-008.06 will inherit the same bound | Product Lead | Before Phase 1 exit |
...
| ~~DEC-012~~ | ~~How long a pre-signup waitlist address is kept~~ | **CLOSED 2026-09-14 — until the one requested message is sent, and never more than 12 calendar months after the grant, whichever comes first.** Owner decision as privacy owner, raised at the STEP-007 close when §27's "waitlist retention period needs privacy-owner approval" was found never to have been answered — and `019` found to have shipped with no retention at all (`BUG-036`). Rejected: a 24-month cap (twice as long for a slow region), and a fixed 12 months regardless of sending (keeps the address after its purpose is exhausted). Expiry deletes the address and keeps the dated grant, as withdrawal does | Nothing further — `020_waitlist_retention.sql`, `expire_waitlist_entries` | Privacy Owner | Resolved |
