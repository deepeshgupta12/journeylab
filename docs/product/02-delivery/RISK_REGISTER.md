...
| **Not limited to Python, and not limited to tests** | **Widened 2026-09-09 at `BUG-033` (`BR-062` §2), the fourteenth reproduction and the first in TypeScript.** `impact(DataTable, upstream)` returned `impactedCount: 0, risk: LOW, "epistemic": "exact"` for a component with **four dependants, three of them not tests** — the barrel `packages/ui/src/index.ts`, `apps/web/.../gallery-client.tsx` (twice) and `apps/web/.../coverage-table.tsx`, **the shipped coverage page**. The description above, and the `RISK-016` row in `CLAUDE.md` §7, both read as though the defect were confined to `apps/api/src/app.py` and to test-module edges. It is not: a TypeScript `LOW` is no more trustworthy than a Python one, and `"epistemic": "exact"` is the word the tool uses to mean the count is not an estimate |
| Mitigation | Until it is fixed, every blast-radius record cross-checks the graph's dependant list against a grep and records **both**. A `LOW` verdict that grep contradicts caps that record's confidence at MEDIUM. `BR-046` §2 is the worked example. **For an exported TypeScript symbol, `tsc --noEmit` is the stronger check** — the compiler is a complete reference check where the graph is not, so `BR-062` records HIGH confidence sourced from the type-checker rather than MEDIUM sourced from the graph |
| Stop condition | None — but no record may cite a `0 dependants` result as evidence of isolation without the cross-check |
| Mechanism, as far as it is now understood | **Refined at STEP-007.04 and confirmed at STEP-007.05.** `BR-061` §2 concluded `apps/api/src/app.py` "has no outgoing edges at all"; that was too strong. `impact(_problem_response, upstream)` resolved its one intra-file caller correctly (`BR-063` §2). What fails is **cross-file edges into the API application**: `impact(get_coverage, upstream)` — defined in `platform_api/coverage.py`, called from `app.py` — returned `0, LOW, "epistemic": "exact"` against **8 call sites**, the fifteenth reproduction (`BR-064` §2). **Sixteenth and seventeenth at `BUG-036`:** `impact(join_waitlist)` and `impact(withdraw_waitlist_consent)` each returned `0, LOW, "epistemic": "exact"`, and grep found both called from the handlers in `app.py` (`BR-065` §2) — three for three on cross-file calls into the API application. **Eighteenth at `BUG-037`:** `impact(HealthChanged.dedupe_key)` returned 0 against the test that read the property (`BR-066` §2). The same record also saw the graph return **all 19** dependants of `coverage_projection` — so the failure is not uniform, which is exactly why a `LOW` verdict can never be taken on trust. The TypeScript case (`BR-062`) shows the same class of miss outside Python entirely. **Nineteenth at the STEP-007 close (2026-09-16), and the first reproduction with an observable correlate:** `context(metres_between)` returned empty `incoming` **and** `outgoing` at `"epistemic": "exact"` against **five references**, including a direct intra-file call at `entity_resolution.py:305` and a call from `tests/ingestion/test_entity_resolution.py`. In the **same** `analyze` run the indexer printed `scope extraction failed for services/ingestion/src/entity_resolution.py: Invalid argument`. **The lead was tested the next day and is REFUTED — recorded here rather than quietly dropped.** A second `analyze` on the identical, clean tree failed extraction on **one** file rather than three, and `entity_resolution.py` scoped **successfully**. `context(metres_between)` still returned empty `incoming` **and** `outgoing` at `"epistemic": "exact"`. The control points the same way: `tests/api/test_api_operations.py` failed extraction **again**, and `TestIdempotency` still held all four of its `has_method` edges. Node and edge counts were **identical across both runs** (11,589 / 17,771) despite different failure sets. **Conclusion: scope-extraction failure is not the mechanism — it neither causes nor even correlates with the missing edges.** What the experiment did establish is a separate defect: **the extraction failures are non-deterministic on an unchanged tree**, so no single `analyze` run's diagnostics describe the indexer's behaviour. **A third run failed on the same three files as the first** — three runs, and the failure set changed twice. **After 19 reproductions the mechanism remains unknown**, and the grep cross-check is still the only defence. The lesson is about method, not about this graph: a correlation observed once, in the same run, was written into repository law before anyone tried to break it |
| Related | `RISK-014` (downgraded — the graph does index Python; this is a different and narrower defect), `BR-046`, `BR-061` §2, `BR-062` §2, `BR-063` §2, `BR-064` §2, `BR-065` §2, `BR-066` §2, the STEP-007 close regression entry (2026-09-16) |
