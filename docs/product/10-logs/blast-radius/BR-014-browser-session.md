...
| **Passkeys not enabled in the tenant** | Sign-in completed with a password; Auth0 offered no passkey enrolment | **Open.** `REQ-SEC-003` says "OIDC with passkey support". The provider supports it (`ADR-013`) but the tenant connection has it switched off: Auth0 → Authentication → Database → the connection → Passkey. Until enabled and exercised, the passkey clause is **unmet**, not merely untested |
