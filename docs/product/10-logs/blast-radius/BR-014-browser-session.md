...
| --- | --- | --- |
| **Auth0 never exercised for real** | No tenant, no credentials in this repository. Tests run against a spec-compliant OIDC shape | **Open and material.** Passkey enrolment, tenant rate limits, and rotation semantics under genuine concurrency are **unproven**. The acceptance criterion "sign-in, refresh and sign-out work" is met against a conforming provider, **not** against Auth0 |
