...
| Unknown / low-confidence areas | Virtualisation libraries frequently break AT row counts — verify the chosen one early |
| Blast radius | BR-017 — scored at execution; **confidence capped while the graph is BLOCKED** |
| Approval required? | Per blast-radius score (HIGH/CRITICAL/low-confidence ⇒ owner approval) |

## 5. Implementation plan
- [ ] Table with correct header scope and caption semantics
- [ ] Keyboard navigation across cells and sortable headers
- [ ] Virtualisation that does not break screen-reader row counts
- [ ] **CSV export** as a first-class primitive capability
- [ ] Responsive collapse to the list primitive
...
- [ ] Table and list pass AA
- [ ] CSV export works from the primitive
- [ ] Virtualisation preserves assistive-technology semantics
...
| Bugs found | — |
| Notes / surprises | Building this before the map is deliberate: REQ-A11Y-003 says no core action may require the map, and that is far cheaper to guarantee than to retrofit. |
